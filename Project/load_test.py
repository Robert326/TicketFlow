...
def simulate_user(user_index):
    ...
    # Loop until no seats are available for this user
    while True:
        try:
            # 1. Get Seats
            ...
            if not available_seats:
                print(f"[{username}] No seats available! Stopping.", flush=True)
                break
            ...
            if res_resp.status_code == 200:
                ...
                print(f"[{username}] Failed to reserve {seat_id} (Already taken). Retrying...", flush=True)
                time.sleep(0.5)
                continue
            else:
